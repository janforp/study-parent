...
public class DelayQueueDemo {
    ...
    static class Cache implements Runnable {
        ...
        private DelayQueue<CacheItem2> delayQueue = new DelayQueue<>();
        ...
        public void put (String key, String value, long exprieTime) {
            CacheItem2 cacheItem = new CacheItem2(key, exprieTime);
        ...
        public void run() {
            while (!stop) {
                CacheItem2 cacheItem = delayQueue.poll();
                if (cacheItem != null) {
                    // 元素过期, 从缓存中移除
    ...
    static class CacheItem2 implements Delayed {
        ...
        public CacheItem2(String key, long exprieTime) {
            this.key = key;
