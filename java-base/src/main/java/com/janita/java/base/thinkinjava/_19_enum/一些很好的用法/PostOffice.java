...
public class PostOffice {
    ...
    static void handle(Mail m) {
        MailHandler[] mailHandlers = MailHandler.values();
        //传入责任链
        for (MailHandler handler : mailHandlers) {
            if (handler.handle(m)) {
                return;
            }
        }
    ...
    public static void main(String[] args) {
        Iterable<Mail> mailIterable = Mail.generator(10);
        for (Mail mail : mailIterable) {
            String details = mail.details();
            print(details);
            handle(mail);
            print("*****");
        }
